...
tidyverse, purrr, ggplot, MuMIn, lmerTest
```
...
#Mutating variables ##The split and map diesn't work!!! Ask Esben maybe
df <- df %>% 
...
#checking the difference by "splitting"/grouping Td from ASD 
df %>%
  split(df$Diagnosis) %>%
  map(summary)
...
ASD: 
n = 176
age = mean: 43.17 months, min: 18.7, max: 62.40
Gender = F: 26, M: 150
Ethnicity = White: 130, African American: 12, Latino: 12, Lebanese: 6, White/Asian: 6, Bangladeshi: 5, other: 5
Difference between verbalIQ1 and verbalIQ?? - from visit 1

TD:
n = 196
age = mean: 30.6 months, min: 18.07, max: 45.07
Gender = F:30, M: 160
Ethnicity = White: 190, Asian: 6
...
```{r}
#Visualizing interaction for model using package "interactions"
interact_plot(m0, pred = VISIT, modx = Diagnosis)
```
...
#Plotting Mean length utterance for all visits for each diagnosis in growth
...
```{r}
#Creating a model
m0 <- lmerTest::lmer(CHI_MLU ~ VISIT*Diagnosis +(1+Diagnosis|SUBJ), df)
...
```{r}
m2 <- lmerTest::lmer(MOT_MLU ~ VISIT*Diagnosis +(1+Diagnosis|SUBJ), df)
