...
head(autism_df)
m0 <- lmerTest::lmer(CHI_MLU ~ Age + Gender + VISIT +Diagnosis + (1|SUBJ), autism_df, REML = F)
#Max model 
...
m1 <- lmerTest::lmer(CHI_MLU ~ Gender + VISIT*Diagnosis + (1|SUBJ), autism_df, REML = F)
#Gender but i cant add random slope, so maybe just delete gender 

#Best model i think ->
m2 <- lmerTest::lmer(CHI_MLU ~ VISIT*Diagnosis + (1+ Diagnosis|SUBJ), autism_df, REML = F)

m3 <- lmerTest::lmer(CHI_MLU ~ VISIT*Diagnosis + (1+ VISIT|SUBJ), autism_df, REML = F)
