...
pacman::p_load(tidyverse, lme4, purrr, lmerTest, effects, MuMIn)
```
...
autism_df <- read_csv("autism_df.csv")
...
- Which variable(s) should be included as fixed factors?
      We put fixed effects as and interaction between visit and diagnosis, because we are evalutating how the kids develop over time given their diagnosis.
- Which variable(s) should be included as random factors?
      As random effects we make a random intercept as SUBJ to account for independence. As a random slop we have diagnosis, to account for the

```{r ex2, include = FALSE}
ggplot(autism_df, aes(x=VISIT, y = CHI_MLU, color = Diagnosis))+
  geom_point()+
  geom_smooth(method = "lm")

ggplot(autism_df, aes(x=SUBJ, y = CHI_MLU)) + 
  geom_boxplot()

head(autism_df)
m0 <- lmerTest::lmer(CHI_MLU ~ Age + Gender + VISIT +Diagnosis + (1+ VISIT|SUBJ), autism_df, REML = F)
#Max model 
  #Age correlates with visit
  #Gender not significant (not equal distributions anyways of F and M)
  #Have to have SUBJ as intercept to account for independence 

m1 <- lmerTest::lmer(CHI_MLU ~ Gender + VISIT*Diagnosis + (1+ VISIT|SUBJ), autism_df, REML = F)
#Gender but i cant add random slope, so maybe just delete gender 

#Best model i think ->
m2 <- lmerTest::lmer(CHI_MLU ~ VISIT*Diagnosis + (1+ VISIT|SUBJ), autism_df, REML = F)

summary(m1)
#plot(effects::allEffects(timem2))
...
r.squaredGLMM(m2)
r.squaredGLMM(m3)
#Cant run the anova because of different predictors 
#anova(m4,m5)
...
m10 <- lmer(CHI_MLU ~ VISIT^2*Diagnosis + (1+Diagnosis|SUBJ), autism_df)


model_new <- lmer(CHI_MLU ~ I(VISIT^2)+ VISIT*Diagnosis + (1+I(VISIT^2)|SUBJ), autism_df, REML=F)
