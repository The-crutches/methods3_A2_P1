...
autism_df <- read_csv("~/Uni stuff/Expmeth3/methods3_A2_P1/autism_df.csv")
```
...
```{r ex2, include = FALSE}
m0 <- lme4::lmer(CHI_MLU ~ Gender + Age +(1+Diagnosis|SUBJ)+
                       VISIT*Diagnosis,
                      autism_df,
                     REML = FALSE)
m1 <- lmerTest::lmer(CHI_MLU ~ Gender+(1+Diagnosis|SUBJ)+
                       VISIT*Diagnosis,
                      autism_df,
                     REML = FALSE)
m2 <- lmer(CHI_MLU ~ VISIT*Diagnosis+(1+Diagnosis|SUBJ),
                      autism_df,
                     REML = FALSE)
m3 <- lmerTest::lmer(CHI_MLU ~ Diagnosis+(1+Diagnosis|SUBJ),
                      autism_df,
                     REML = FALSE)
m4 <- lmerTest::lmer(CHI_MLU ~ VISIT+(1+Diagnosis|SUBJ),
                      autism_df,
                     REML = FALSE)
...
growth <- lmerTest::lmer(CHI_MLU ~ VISIT*Diagnosis+(1+VISIT|SUBJ)+
                      I(VISIT^2),
                      autism_df,
                     REML = FALSE)
...
test <- lmer(Age ~ Gender+ (1| SUBJ), autism_df, REML = FALSE)
summary(test)
