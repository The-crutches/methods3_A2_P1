...
growth <- lmer(CHI_MLU ~ VISIT*Diagnosis + (1+VISIT|SUBJ) + I(VISIT^2),
               df, REML = FALSE)
...
a <- select(df3,  'SUBJ', 'VISIT', 'Diagnosis', 'CHI_MLU')
a$predicted <- predict(growth)
a$residuals <- residuals(growth)

a <- a %>%
  mutate(SUBJ = (as.factor(SUBJ)),
         VISIT = (as.integer(a$VISIT)))

ggplot(a, aes(x = VISIT, y = CHI_MLU)) +
...
ggplot(df3, aes(CHI_MLU, fit)) + geom_point() + geom_smooth(method = lm)
